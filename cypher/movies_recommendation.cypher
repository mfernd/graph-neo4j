...
// ---------------------------------------------------------------
// Query #1: movie recommendations based on similarity
// Get movie recommendations based on (direct) friend likes
MATCH (user:User {name: "Matéo"})-[:FRIENDS_WITH]->(friend:User)
WHERE user <> friend // prevent cycling
WITH user, friend
MATCH (friend)-[:LIKES]->(movie:Movie) // retrieves movies liked by their direct friend
WHERE NOT (user)-[:LIKES]->(movie) // excludes movie already liked by the user
RETURN movie.title, 
...
WHERE NOT (u)-[:LIKES]->(m) // excludes movie already liked by the user
RETURN m.title;
