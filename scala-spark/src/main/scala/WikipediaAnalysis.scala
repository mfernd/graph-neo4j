...
object WikipediaAnalysis extends App {
  ...
  val rawData =
    spark.sparkContext.textFile(f"file:///${SparkFiles.get("wikipedia.dat")}")
  ...
  def parseWikiPage(pageString: String): WikiPage = {
    ...
    val title =
      titleRegex.findFirstMatchIn(pageString).map(_.group(1)).getOrElse("")
    val text =
      textRegex.findFirstMatchIn(pageString).map(_.group(1)).getOrElse("")
